...
4. 각 추천 음식에 대한 **근거가 되는** 출처 정보 (source) - **반드시 과학적 근거(논문, 연구 결과 등) 또는 공신력 있는 기관(정부 건강 부처, 주요 학회 등)의 공식 발표 자료를 기반으로 작성**하고, 해당 **외부 링크(URL)를 정확하게** 포함해주세요. 만약 **검증 가능한 공식 출처가 없다면**, \'검증된 공식 출처 없음\'으로 명확히 표시하고, **개인 블로그나 일반 웹사이트 링크는 절대 포함하지 마세요**. 출처 링크가 외국 사이트일 경우 원문 그대로 제공하고, 언어 선택이 가능하다면 영문(en) 버전을 우선으로 제공해주세요.
...
- source: 해당 추천의 검증 가능한 공식 출처 (URL 또는 '검증된 공식 출처 없음')
...
예시 응답 형식:
{
  ...
  "recommendations": [
    ...
    },
    {
      ...
      "source": "https://pubmed.ncbi.nlm.nih.gov/XXXXXXXX/" // 예시: 논문 링크
    }
  ]
}
